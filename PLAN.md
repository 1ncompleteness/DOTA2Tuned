...
- 2026-06-15 05:34: researched Safari's native video start overlay and confirmed it is browser-owned, with older WebKit pseudo-elements only a best-effort hide path because newer Safari/iOS controls can live in closed UA shadow DOM. Added best-effort `::-webkit-media-controls-*` suppression for the loader video, kept the custom loader button as the reliable control, and shortened its visible label from `Play Video` to `Play`.

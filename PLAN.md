...
- 2026-06-15 06:58: replaced Evidence/Prediction accordions with direct `gr.Code` blocks so the generated `.label-wrap` accordion button is not rendered, while keeping the top-panel code toolbar background transparent.
- 2026-06-15 07:16: removed rich icon/tag injection from opened dropdown option lists, kept selected-value icons in closed fields, added selected-option checkmarks, hid empty preview `gr.HTML` containers with `visible=False` updates, made hero-card remove buttons truly circular, and added THN-inspired scramble-resolve plus red/gold glitch-settle animation for dynamic text across previews, assistant outputs, build cards, and markdown result regions.
- 2026-06-15 08:22: restored rich dropdown option rows with hero/item icons and hero role tags, added a leading selected checkmark inside those rows, hid duplicate selected labels in the dropdown input while preserving visible typed search text and the blinking caret, replaced Gradio processing/time trackers with static module status strips, and matched the code toolbar top-panel background to its parent with borders removed.
- 2026-06-15 08:41: removed the static module status strips, restored Gradio's processing/time tracker while rewriting `Processing` to `Thinking`, added module explanation intros, expanded tuned-model sample prompts, removed selected hero text/icons from closed hero dropdown fields without changing rich dropdown lists, restored readable role/scope field text, and removed residual code-toolbar button borders.
- 2026-06-15 08:58: collapsed Gradio's selected hero/item token DOM inside closed dropdown controls by preserving selected labels in `data-*`, clearing the internal token text spans, hiding token icons, and keeping the existing preview/result divs as the visible selected-content surface.
- 2026-06-15 09:16: hid Gradio's native selected-option check span inside every decorated hero/item dropdown row so only the custom left checkmark is visible. Added stable ids for Hero Meta hero/item and Builds hero dropdowns, refreshed hidden single-select state before option decoration, and verified Builds remains single-selection after sequential hero picks.
- 2026-06-15 13:23: extended the startup loader with a Trajan-styled red/gold loading ring and module-inspired scramble/glitch ready text. The montage video now plays once for its full duration; once app assets are ready, the ring/text becomes a clickable Skip control, while automatic entry waits for video end.
- 2026-06-15 13:43: fixed rapid-selection text animation races by moving scramble frames into a non-mutating overlay and adding sequence/cancel state, so hero-card text cannot settle as randomized garbage. Reworked Draft Coach, Hero Meta, Match Predictor, and Draft Lab selector layouts so each dropdown owns the preview block directly below it instead of using separate left-filled preview rows.
- 2026-06-15 13:48: converted the startup loader ring from a spinner into a determinate red/gold conic progress meter with center percentage text and a dot at the current completion angle. Slowed loader and module text randomizer frames so text resolves visibly left-to-right instead of appearing all at once.
- 2026-06-15 14:01: moved the loader progress control to the bottom center of the viewport, underlined its status text, and changed the ready copy to `Loading completed · Click to skip`. Recalibrated loader progress with a drift loop and higher readiness milestones so it moves through intermediate percentages instead of jumping from the mid-50s to 100, and reset the conic arc to start at the top axis instead of the left.
- 2026-06-15 14:07: reversed the determinate loader ring gradient so the filled arc now runs from Dota gold to Dota red while keeping the progress angle, center percentage, and endpoint dot behavior unchanged.
- 2026-06-15 14:18: moved loader status text animation to the same non-mutating left-to-right scramble overlay used by module text. Both `Loading platform` and `Loading completed · Click to skip` now visibly randomize before settling with the red/gold pulse.
